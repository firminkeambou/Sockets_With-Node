...
const apiServer = require('./api'); // Import the API server from api.js
...
const httpServer = http
  ...
  (apiServer); // here api(express) is being passed as a request handler to the server, so that the same server can handle both API requests and WebSocket connections. This way, we can avoid issues with CORS and have a single entry point for both the API and the WebSocket connections.
